...
%Рассмотрим явление дышащего хаоса, которое было описано в статье [?] в симметричном случае для случая асимметричного клина.
...
%f(x_0+\Delta \bar{x},y_0+\Delta \bar{y})
%f(\bar{x},\bar{y})=f(\xi (x_0+\Delta x,y_0+\Delta y), \eta (x_0+\Delta x,y_0+\Delta y))=f(x_0,y_0)+f_x\Delta \bar{x}+f_y\Delta \bar{y}+ \dfrac{1}{2}f_{x^2}\Delta \bar{x}^2+f_x\Delta \bar{x}\Delta \bar{y}+\dfrac{1}{2}f_{y^2}\Delta \bar{y}^2+...
...
%Откуда теперь можем записать и коэффициенты при $f_{xy}$ из \eqref{GrindEQ_b_kh_7} подставляя \eqref{GrindEQ_b_kh_10}
%Однако, если при определенных параметрах коэффициент при $f_{xy}$ оказывается равным нулю, т.е.
...
\end{array}
%D(d,\tau,\alpha)=\frac{2\cos 2 \alpha  \left(d-(\tau  \cos \alpha-d \sin\alpha)^2\right)}{d }-d+2, T(d,\tau ,\alpha )=D(d,\tau,\alpha)\tg \alpha +\sqrt{\frac{D(d,\tau,\alpha)}{d}} (\tau -d \tg \alpha)
\end{equation}

%\begin{equation} \label{GrindEQ_b_kh_12}
%T(d,\tau ,\alpha )=D(d,\tau,\alpha)\tg \alpha +\sqrt{\frac{D(d,\tau,\alpha)}{d}} (\tau -d \tg \alpha)
%\end{equation}
...
%Вычисляя первые производные в этой точке для \eqref{GrindEQ_b_kh_11}
\begin{equation}
\begin{array}{lr}
...
\end{array}
\end{equation}
% \eqref{GrindEQ_b_kh_12} получим из \eqref{GrindEQ_b_kh_8}:
